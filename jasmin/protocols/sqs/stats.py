from jasmin.tools.singleton import Singleton
from jasmin.tools.stats import Stats

class SQSStatistics(Stats):
    """SQS statistics holder"""

    def __init__(self, api_id):
        self.api_id = api_id

        self.init()

    def init(self):
        self._stats = {
            'created_at': 0,
            'request_count': 0,
            'last_request_at': 0,
            'auth_error_count': 0,
            'route_error_count': 0,
            'interceptor_error_count': 0,
            'interceptor_count': 0,
            'throughput_error_count': 0,
            'charging_error_count': 0,
            'server_error_count': 0,
            'success_count': 0,
            'last_success_at': 0,
        }

    def getStats(self):
        return self._stats


class SQSStatsCollector(metaclass=Singleton):
    """SQS statistics collection holder"""
    apis = {}

    def get(self):
        """Return a SQS's stats object or instanciate a new one"""
        api_id = 'main'
        if api_id not in self.apis:
            self.apis[api_id] = SQSStatistics(api_id)

        return self.apis[api_id]
